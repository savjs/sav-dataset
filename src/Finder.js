...
Finder.prototype.$and = function(results, args, sources) {
    ...
    var fullSearch = true; // 全文检索
    if (indexs) {
        //1. 如果条件中包含索引或主键, 提取出来并获得结果集
        var _indexs = this._collection._indexs,
            item;
    ...
    // 2. 全文检索
    var compares = args.compares;
    if (compares && compares.length) {
        results = results.filter(function(it) {
            for (var i = 0, l = compares.length; i < l; ++i) {
                var info = compares[i];
...
Finder.prototype.$limit = function(results, args, sources) {
    var offset = args.length == 2 ? args[0] : 0;
    var limit = args.length > 1 ? args[1] : args[0];
...
var rawKeys = ['$sort', '$count', '$alias'];

function parseQuery(opts) {
    var keys = Object.keys(opts);
    var andCond = keys.filter(function(key) {
        return key[0] != '$'
    }).map(function(key) {
